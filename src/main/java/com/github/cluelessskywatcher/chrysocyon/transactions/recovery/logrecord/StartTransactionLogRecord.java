...
public class StartTransactionLogRecord implements RecoveryLogRecord {
    private @Getter int transaction;

    public StartTransactionLogRecord(PageObject p) {
    ...
    public RecoveryLogRecordType type() {
        return RecoveryLogRecordType.START_TRANSACTION;
    }
    ...
    public static int writeToLog(AppendLogManager logManager, int transaction) {
        ...
        p.setInt(RecoveryLogRecordType.START_TRANSACTION.getCode(), 0);
